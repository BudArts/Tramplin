      ...
      <div className="admin-reviews__toolbar">
        ...
        <div className="admin-reviews__filters">
          {statusOptions.map(option => (
            <button
              key={option.value}
              className={`admin-reviews__filter-btn ${statusFilter === option.value ? 'active' : ''}`}
      ...
      <div className="admin-reviews__list">
        {filteredReviews.length === 0 ? (
          <div className="admin-reviews__empty">
            <div className="admin-reviews__empty-icon"><MessageSquare size={48} /></div>
          ...
          filteredReviews.map((review, index) => (
            <motion.div
              key={review.id}
              className={`admin-reviews__card ${review.is_hidden ? 'admin-reviews__card--hidden' : ''}`}
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ delay: index * 0.02 }}
            >
              <div className="admin-reviews__card-header">
                <div className="admin-reviews__card-company">
                  {review.company?.logo_url ? (
                    <img src={review.company.logo_url} alt={review.company.name} />
                  ) : (
                    <div className="admin-reviews__card-company-placeholder">
                      {review.company?.name?.[0] || '?'}
                    </div>
                  )}
                  <div>
                    <div className="admin-reviews__card-company-name">
                      {review.company?.display_name || review.company?.name}
                    </div>
                    <div className="admin-reviews__card-author">
                      {review.is_anonymous ? (
              ...
              {review.title && (
                <h3 className="admin-reviews__card-title">{review.title}</h3>
              )}

              {review.text && (
                <p className="admin-reviews__card-text">{review.text}</p>
              )}

              {(review.pros || review.cons) && (
                <div className="admin-reviews__card-pros-cons">
                  {review.pros && (
                    <div className="admin-reviews__card-pros">
                      <strong>Плюсы:</strong> {review.pros}
                    </div>
                  )}
                  {review.cons && (
                    <div className="admin-reviews__card-cons">
              ...
              <div className="admin-reviews__card-footer">
                ...
                <div className="admin-reviews__card-badges">
                  {review.is_verified && (
                    <div className="admin-reviews__badge admin-reviews__badge--verified">
                      <CheckCircle size={12} />
                      <span>Подтвержден</span>
                    </div>
                  )}
                  {review.is_hidden && (
                    <div className="admin-reviews__badge admin-reviews__badge--hidden">
                      <AlertCircle size={12} />
                      <span>Скрыт</span>
                    </div>
                  )}
              ...
              <div className="admin-reviews__card-actions">
                {!review.is_verified && !review.is_hidden && (
                  <button
                    className="admin-reviews__action-btn admin-reviews__action-btn--verify"
                ...
                {!review.is_hidden ? (
                  <button
                    className="admin-reviews__action-btn admin-reviews__action-btn--hide"
                  ...
                  <button
                    className="admin-reviews__action-btn admin-reviews__action-btn--restore"
