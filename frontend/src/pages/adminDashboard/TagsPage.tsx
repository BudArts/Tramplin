...
const TagsPage = () => {
  ...
  if (loading) {
    return (
      <div className="admin-tags__loading">
  ...
  return (
    <div className="admin-tags">
      ...
      <div className="admin-tags__toolbar">
        <div className="admin-tags__search">
          <Search size={20} />
          <input
            type="text"
      ...
      <div className="admin-tags__tabs">
        <button
          className={`admin-tags__tab ${activeTab === 'pending' ? 'active' : ''}`}
          onClick={() => setActiveTab('pending')}
        >
          На модерации
          {pendingTags.length > 0 && (
            <span className="admin-tags__tab-count">{pendingTags.length}</span>
          )}
        </button>
        <button
          className={`admin-tags__tab ${activeTab === 'approved' ? 'active' : ''}`}
          onClick={() => setActiveTab('approved')}
        >
          Все теги
          <span className="admin-tags__tab-count">{approvedTags.length}</span>
        </button>
      </div>
      ...
      {activeTab === 'pending' && (
        <div className="admin-tags__list">
          {filteredPending.length === 0 ? (
            <div className="admin-tags__empty">
              <div className="admin-tags__empty-icon"><Tag size={48} /></div>
            ...
            filteredPending.map((tag, index) => (
              <motion.div
                key={tag.id}
                className="admin-tags__card admin-tags__card--pending"
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ delay: index * 0.03 }}
              >
                <div className="admin-tags__card-content">
                  <div className="admin-tags__card-name">{tag.name}</div>
                  <div className="admin-tags__card-category">{getCategoryLabel(tag.category)}</div>
                  <div className="admin-tags__card-date">Предложен: {formatDate(tag.created_at)}</div>
                </div>
                <div className="admin-tags__card-actions">
                  <button
                    className="admin-tags__action-btn admin-tags__action-btn--approve"
                  ...
                  <button
                    className="admin-tags__action-btn admin-tags__action-btn--reject"
                    onClick={() => {
                      setSelectedTag(tag);
            ...
            filteredApproved.map((tag, index) => (
              ...
              >
                <div className="admin-tags__card-content">
                  <div className="admin-tags__card-name">{tag.name}</div>
                  <div className="admin-tags__card-category">{getCategoryLabel(tag.category)}</div>
                  <div className="admin-tags__card-usage">
