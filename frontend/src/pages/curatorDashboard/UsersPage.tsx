// frontend/src/pages/curatorDashboard/UsersPage.tsx
...
const UsersPage = () => {
  ...
  if (loading) {
    return (
      <div className="curator-users__loading">
  ...
  return (
    <div className="curator-users">
      <motion.div
        className="curator-users__header"
      ...
      <div className="curator-users__filters">
        <div className="curator-users__search">
          <Search size={20} />
          <input
            type="text"
        ...
        <select
          className="curator-users__filter-select"
          value={roleFilter}
      ...
      <div className="curator-users__list">
        {filteredUsers.length === 0 ? (
          <div className="curator-users__empty">
            <div className="curator-users__empty-icon"><Users size={48} /></div>
          ...
          filteredUsers.map((user, index) => {
            ...
            const statusConfig = getStatusLabel(user.status);
            return (
              <motion.div
                key={user.id}
                className="curator-users__card"
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ delay: index * 0.02 }}
                onClick={() => handleUserClick(user)}
              >
                <div className="curator-users__card-avatar">
                  {user.avatar_url ? (
                    <img src={user.avatar_url} alt={`${user.first_name} ${user.last_name}`} />
                  ) : (
                    <div className="curator-users__card-avatar-placeholder">
                      {user.first_name?.[0]}{user.last_name?.[0]}
                    </div>
                  )}
                </div>
                <div className="curator-users__card-info">
                  <div className="curator-users__card-header">
                    <h3 className="curator-users__card-name">
                      {user.display_name || `${user.first_name} ${user.last_name}`}
                    </h3>
                    <div className="curator-users__card-badges">
                      <div className="curator-users__card-role" style={{ backgroundColor: `${roleConfig.color}20`, color: roleConfig.color }}>
                        <RoleIcon size={12} />
                        <span>{roleConfig.label}</span>
                      </div>
                      <div className="curator-users__card-status" style={{ backgroundColor: `${statusConfig.color}20`, color: statusConfig.color }}>
                        {statusConfig.label}
                      </div>
                      {user.is_email_verified && (
                        <div className="curator-users__card-verified">
                          <CheckCircle size={12} />
                        </div>
                      )}
                    </div>
                  </div>
                  <div className="curator-users__card-details">
                    <div className="curator-users__card-detail">
                      <Mail size={12} />
                ...
                <div className="curator-users__card-actions">
                  <button className="curator-users__card-btn">
                    <Eye size={16} />
                    <span>Просмотр</span>
                  </button>
                </div>
      ...
      <AnimatePresence>
        {modalOpen && selectedUser && (
          ...
          >
            ...
            >
              <button className="curator-modal__close" onClick={() => setModalOpen(false)}>
                <X size={20} />
              </button>

              <div className="curator-modal__header">
                <div className="curator-modal__avatar">
                  {selectedUser.avatar_url ? (
                    <img src={selectedUser.avatar_url} alt="" />
                  ) : (
                    <div className="curator-modal__avatar-placeholder">
                ...
                <div className="curator-modal__info">
                  {!isEditing ? (
                    <>
                      ...
                      <p className="curator-modal__email">{selectedUser.email}</p>
                      <div className="curator-modal__badges">
                        <div className="curator-modal__role-badge">
                    ...
                    <div className="curator-modal__edit-form">
                      <input
                        type="text"
                        placeholder="Имя"
              ...
              {!isEditing ? (
                <>
                  <div className="curator-modal__tabs">
                    <button className="curator-modal__tab active">Личная информация</button>
                    {selectedUser.role === 'student' && <button className="curator-modal__tab">Образование</button>}
                    {selectedUser.role === 'company' && <button className="curator-modal__tab">Компания</button>}
                  </div>
                  ...
                  <div className="curator-modal__tab-content">
                    <div className="curator-modal__details">
                      ...
                      {selectedUser.bio && (
                        <div className="curator-modal__detail-section">
                          <h4>О себе</h4>
                          <p className="curator-modal__bio">{selectedUser.bio}</p>
                        </div>
                      )}
                ...
                <div className="curator-modal__edit-section">
                  {selectedUser.role === 'student' && (
                    <>
                      <h4>Образование</h4>
                      <input
              ...
              <div className="curator-modal__actions">
                {!isEditing ? (
                  <>
                    <button
                      className="curator-modal__action-btn curator-modal__action-btn--edit"
                    ...
                    <select
                      className="curator-modal__role-select"
                  ...
                  <>
                    ...
                    <button
                      className="curator-modal__action-btn curator-modal__action-btn--save"
                      onClick={editUser}
                      disabled={actionLoading}
                    >
